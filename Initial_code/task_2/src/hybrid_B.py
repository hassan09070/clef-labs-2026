...
# 1. Config 
# Secured: Set your GROQ_API_KEY environment variable before running
GROQ_API_KEY  = ""
...
CHUNK_OVERLAP = 20
TOP_K         = 3
OUTPUT_FILE   = "polyfiqa_easy_rag_results_changeB.json"
...
# 4. Build Structured Chunks (Change A - Full Context) 
def build_structured_chunks(sections: dict) -> list[dict]:
    ...
    for source in ["balance_sheet", "cash_flow", "income_statement"]:
        ...
        for line in lines:
            ...
            # Fixed separator detection: skips purely structural borders
        ...
        # Inject full table header into every chunk
...
def retrieve_chunks(chunks: list[dict], question: str, top_k: int = TOP_K, alpha: float = 0.5) -> tuple[str, list[str]]:
    ...
    # Sparse Retrieval (BM25 for exact number/keyword matching)
    tokenized_corpus = [t.lower().split() for t in texts]
    bm25 = BM25Okapi(tokenized_corpus)
    tokenized_query = question.lower().split()
    bm25_scores = np.array(bm25.get_scores(tokenized_query))

    # Dense Retrieval (e5-small for semantic matching)
...
def get_prompt(question: str, retrieved: str, instructions: str) -> tuple[str, str]:
    ...
    if "revenue" in q and "trend" in q:
        ...
        hint = "Extract current assets, total liabilities, total equity. Calculate liabilities/equity. Show only final numbers."
        ...
        hint = "Extract R&D expenditure and revenue. Calculate R&D/revenue ratio. Show only the final ratio."
    else:
...
# 8. Groq Call With Backoff 
async def call_groq(client: AsyncGroq, system: str, user: str) -> str:
    if not GROQ_API_KEY:
        print("API Key missing! Set GROQ_API_KEY environment variable.")
        return ""
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if any(x in err for x in ["rate limit", "429", "413"]):
                # FIX: Stop exponential backoff. Just wait for the minute to reset.
                print(f"    TPM limit hit. Waiting 60s for Groq bucket to clear...")
...
async def main():
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        json.dump({
            "model":    MODEL,
            "approach": "hybrid_rag_bm25_multilingual_e5_small",
            "results":  all_results

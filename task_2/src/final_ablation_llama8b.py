...
TEMPERATURE   = 0.0
MAX_RETRIES   = 10
TOP_K         = 3     # Kept at 3 to minimize payload size
OUTPUT_FILE   = "polyfiqa_expert_final_ablation_llama_8b.json"
...
def parse_query(query: str) -> dict:
    ...
    # Safely find markdown separators without triggering double-dash rules
    seps = []
    ...
    # Fallback if separators aren't perfectly matched
    if len(seps) < 3:
...
# === 4. Build Structured Chunks (Change A) ===
def build_structured_chunks(sections: dict) -> list[dict]:
    ...
    for source in ["balance_sheet", "cash_flow", "income_statement"]:
        ...
        for line in lines:
            ...
            # Skip purely structural borders
...
# === Step 5: Prompt Builder (Expert Multi-Hop Surgery) ===
def get_prompt(question: str, retrieved: str, instructions: str) -> tuple[str, str]:
    
    # Surgical Hint tailored perfectly to the Expert analytical questions
    ...
    safe_retrieved = retrieved[:12000] 
    if len(retrieved) > 12000:
        print("    [Warning] Context truncated to survive free-tier TPM.")
...
# === 8. Groq Call With Flat Rate Limit Handling ===
...
async def process_event(
    ...
    async with semaphore:
        for q_idx, (question, reference) in enumerate(
            ...
            # Strict 35-second sleep to ensure we stay under Groq API limits
...
async def main():
    ...
    print(f"\nRunning Final Ablation RAG with {MODEL}...\n")
    tasks = [
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        json.dump({
            "model":    MODEL,
            "approach": "final_ablation_llama_8b",
            "results":  all_results

# polyfiqa_rag_baseline.py
# Structure-aware RAG for PolyFiQA-Easy (Task 2, FinMMEval @ CLEF 2026)
...
CHUNK_OVERLAP = 20
TOP_K         = 6
OUTPUT_FILE   = "polyfiqa_easy_rag_results_changeA_llama_8B_v2.json"
...
def parse_query(query: str) -> dict:
    ...
    return {
        ...
        # seps[3] is trailing divider — ignored

        # 5 news articles — kept whole (only ~3 lines each)
...
# ── Step 2: Build Structured Chunks ──────────────────────────────────
def build_structured_chunks(sections: dict) -> list[dict]:
    all_chunks = []

    # 1. Chunk financial tables (Change A Logic)
    for source in ["balance_sheet", "cash_flow", "income_statement"]:
        ...
        for line in lines:
            ...
            # Only skip rows that are PURELY separator characters with no real content
        ...
        for i in range(0, len(data_rows), 4):
            # Find the date row specifically (contains years like 2020, 2019)
            date_row = ""
            for row in data_rows[:3]:  # date is usually in first few rows
                if any(year in row for year in ['2020', '2019', '2021', '2018']):
                    date_row = row
                    break

            # Inject date context only — much less repetitive
            context_prefix = f"[Date context: {date_row}]\n" if date_row else ""
            chunk_text = context_prefix + '\n'.join(data_rows[i:i+4])
            all_chunks.append({"source": source, "text": chunk_text})

    # 2. Keep news articles whole (Required for Evidence)

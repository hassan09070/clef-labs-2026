...
def get_prompt(question: str, retrieved: str, instructions: str) -> tuple[str, str]:
    ...
    if "revenue" in q and any(x in q for x in ["trend", "growth", "change", "over"]):
    ...
    elif any(x in q for x in ["cash flow", "operating", "investing", "financing"]):
        hint = "Extract operating, investing, and financing cash flows. Identify irregularities."
    elif any(x in q for x in ["r&d", "research and development"]):
        hint = "Extract R&D expenditure and total revenue. Calculate R&D/revenue ratio. Show final ratio only."
    elif any(x in q for x in ["margin", "gross profit"]):
        hint = "Extract gross profit and revenue. Calculate the gross margin percentage."
    else:
        hint = "Answer based on the financial data and news provided. Quote exact figures from the statements."
    ...
    CRITICAL RULES:
    1. NEWS: Copy EXACT text from the labeled news articles. Do NOT paraphrase.
    2. FINANCIALS: You MUST explicitly quote the exact numerical figures from the tables.
    3. NUMBER FORMATTING: You MUST convert all large currency figures into Billions formatted to one decimal place with a 'B' suffix (e.g., convert 35,000 million to $35.0B). Format all percentages to one decimal place (e.g., 14.0%). Round all ratios to two decimal places (e.g., 1.49).
    4. If no relevant evidence is found, write "None".

    Step 1: Perform all calculations and multi-hop reasoning inside <scratchpad> tags.
    Step 2: Output your final response strictly using these XML tags:
    <answer>
    [Your 1-2 sentence final answer here, including the exact extracted numbers or calculated ratio]
    </answer>
    <evidence>
...
async def main():
    ...
    parser.add_argument("-output", default="reranked_easy.json")
    ...
    # dataset = load_dataset("TheFinAI/PolyFiQA-Easy", split="test") # or Expert
    ...
    tasks = [process_event(client, tid, event_map[tid], semaphore, "easy") for tid in ordered_ids]
